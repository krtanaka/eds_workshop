rm(list=ls())
# Library Calls and Function Defiintion -----------------------------------
...
# Island, Parameter, and Time Series Summary Definitions ------------------
EDSpath="M:/Environmental Data Summary/"
#  "C:/Users/Thomas.Oliver/WORK/Projects/Environmental Data Summary/"

Ibbox=read.csv(paste0(EDSpath,"GeographyInputFiles/Island_10km_Grid_Extents_CSV_Hawaii2019Bleaching.csv"),stringsAsFactors = F)
uI=unique(Ibbox$ISLAND.CODE)

ParamDF=read.csv(paste0(EDSpath,"ParameterInputFiles/EDS_parameters_Hawaii2019Bleaching.csv"),stringsAsFactors = F)
ParamDF=subset(ParamDF,DOWNLOAD=="YES")
startwith=1
endwith=nrow(ParamDF)
ParamDF=ParamDF[startwith:endwith,]
uP=unique(ParamDF$PARAMETER.NAME)
...
for (iP in 1:length(uP)){
  ...
  #thisp$START_DATE=mdy(thisp$START_DATE)
  #thisp$STOP_DATE=mdy(thisp$STOP_DATE)
  print(paste0("--------------------------------------------------------"))
  print(paste0("----------   ",thisp$PARAMETER.NAME,"   ----------"))
  print(paste0("--------------------------------------------------------"))
  ...
  if (thisp$FREQUENCY=="Climatology"){
    # print(paste0("Accessing ",thisp$PARAMETER.NAME," climatology...(no time dimensions)"))
    #For each island - set up by block folder
    ...
    for (ii in 1:length(uI)){
      ...
      #print(paste0("Accessing ",thisp$PARAMETER.NAME," climatology at ",thisisland$ISLAND.CODE))
      ...
      if(!file.exists(targetfilename)){
        thisIP=griddap(x = thisp$DATASET.ID,url=thisp$URL,
                       fields = c(thisp$GRID.VARIABLE),
                       #time=c('first','last'),
                       longitude=thislong,
                       latitude=thisisland[,c("BOTTOM_YMIN","TOP_YMAX")],
                       fmt="nc",store=disk(path=pib_path))
        ncstatus=file.rename(thisIP$summary$filename,
                             targetfilename)
        print(paste0(thisisland$ISLAND.CODE," written to disk. ",ii,' of ',length(uI)))
      }
    }# All Island Closes
    print(paste0("Completed ",thisp$PARAMETER.NAME," Check: ",length(uI)," islands data present. "))    
    
    MergedRasterFileName=paste0(paramoutpath,"/",uP[iP],"_",thisp$START_DATE,"_AllIslands.nc")
    ...
    for (ii in 1:length(uI)){
      ...
      #print(paste0("Accessing ",thisp$PARAMETER.NAME," time series at ",thisisland$ISLAND.CODE))
      ...
      testIP=griddap(x = thisp$DATASET.ID,url=thisp$URL,
      ...
      Nblocks=ceiling((singlestep*thisp$TIMESTEPS)/thisp$BLOCKSIZE)
      # Get Metadata
      ...
      # ts_start=ymd_hms(NCG[which(NCG[,"attribute_name"]=="time_coverage_start"),"value"])
      # ts_end=ymd_hms(NCG[which(NCG[,"attribute_name"]=="time_coverage_end"),"value"])
      ...
      for(blockI in 1:Nblocks){
        ...
        if(!file.exists(targetfilename)){
          ...
          repeat{
            thisIP=tryCatch({griddap(x = thisp$DATASET.ID,
                                    ...
                                    # fields = c(thisp$GRID.VARIABLE),
                                     ...
                                     longitude=thislong,
                                     latitude=thisisland[,c("BOTTOM_YMIN","TOP_YMAX")],
                                     fmt="nc",store=disk(path=pib_path),read=FALSE)},
                            error=function(e){
                              print("GRIDDAP ERROR")
                            }
            )
            ...
            if(Ntried>Ntries){
              ...
              Ntried=Ntried+1
              if(Ntried>3){#After a few tries, try deleting any "scrambled" filenames in the directory
          ...
          ncstatus=file.rename(thisIP$summary$filename,
                               targetfilename)
          print(paste0(thisisland$ISLAND.CODE,", block #",blockI," of ",Nblocks," written to disk. Island #",ii,' of ',length(uI)))
        }#end target exists IF
      }#end block loop for
      print(paste0("Completed ",thisp$PARAMETER.NAME," Check: ",Nblocks," blocks present for ",thisisland$ISLAND.CODE,". ",ii," of ",length(uI)," islands."))    
      ...
      outfile=paste0(pi_path,"/",thisisland$ISLAND.CODE,"_",
                     thisp$PARAMETER.NAME,"_",
                     floor_date(ts_start,unit = "day"),"_",
                     floor_date(ts_end,unit = "day"),".nc")
      if(!file.exists(outfile)){
        AllBlock=list.files(pib_path,full.names = T,pattern = thisisland$ISLAND.CODE)
        out=merge_times_nc_list(infilenamelist = as.list(AllBlock),
                                variable_name = thisp$GRID.VARIABLE,
                                outfilename = outfile)      
      }  
      print(paste0("Completed ",thisp$PARAMETER.NAME," Check: Merged time-series .nc present for ",thisisland$ISLAND.CODE))    
      ...
      for(sumi in 1:length(TsSummaries)){
        ...
        if(!file.exists(targetfilename)){
          ...
          if (thisSum %in% c("mean","q05","q95")){
            island_sum=apply(island_ts_xyt,c(1,2),thisSum,na.rm=T)
          }else if (thisSum %in% c("mean_annual_range","mean_monthly_range","mean_biweekly_range")){
            island_sum=apply(island_ts_xyt,c(1,2),thisSum,t=island_ts_t,na.rm=T)
          }else if (thisSum %in% c("DHW.Np10y","DHW.MeanMax","DHW.MeanDur","DHW.MaxMax","DHW.CI95Max",
                                   "DHW.Np10y_Major","DHW.MeanMax_Major","DHW.MeanDur_Major","DHW.MaxMax_Major","DHW.CI95Max_Major")){
            island_sum=apply(island_ts_xyt,c(1,2),thisSum,t=island_ts_t,na.rm=T)
          }  
      ...
      print(paste0("Completed ",thisp$PARAMETER.NAME," Check: Summary files output for ",
                   thisisland$ISLAND.CODE,": ",paste(TsSummaries,collapse="; ")))    
    ...
    for(sumi in 1:length(TsSummaries)){
      ...
      raster_outfile=paste0(ds_path,"/",uP[iP],"_",thisSum,"_",thisp$START_DATE,"_",thisp$STOP_DATE,"_AllIslands.nc")
